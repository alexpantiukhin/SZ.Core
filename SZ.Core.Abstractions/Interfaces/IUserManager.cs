...
namespace SZ.Core.Abstractions.Interfaces
{
    public interface IUserManager
    {
        ...
        /// <param name="userSessionEnvironment">окружение, содержащее текущий Identity</param>
        /// <returns></returns>
        Task<User> GetCurrentUserAsync([NotNull] DBProvider provider, [NotNull] ISZScopeEnvironment userSessionEnvironment);
        /// <summary>
        ...
        /// </summary>
        /// <param name="provider">Провайдер БД</param>
        /// <param name="userSessionEnvironment">Окружение с текущим пользователем</param>
        /// <param name="userId">Id пользователя, которому генерируется новый пароль</param>
        /// <returns>Новый пароль</returns>
        Task<Result<string>> GeneratePasswordAsync([NotNull] DBProvider provider, [NotNull] ISZScopeEnvironment userSessionEnvironment, Guid userId);
    }
}
